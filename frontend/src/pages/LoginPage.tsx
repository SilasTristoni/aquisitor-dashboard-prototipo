...
export default function LoginPage() {
  ...
  useEffect(() => {
    api<{ demo_credentials: { email: string; password: string } | null }>("/build-info")
      .then((information) => {
        if (!information.demo_credentials) return;
        setEmail(information.demo_credentials.email);
        setPassword(information.demo_credentials.password);
  ...
  return <main className="login-shell">
    ...
    <section className="login-form-wrap"><form className="login-card" onSubmit={submit}><div className="login-icon"><LockKeyhole /></div><p className="eyebrow">ACESSO SEGURO</p><h2>Bem-vindo de volta</h2><p className="form-intro">Entre para acessar o ambiente de monitoramento.</p>
      ...
      {demoCredentialsLoaded && <div className="demo-credentials"><CheckCircle2 /><div><strong>Ambiente de demonstração</strong><span>Credenciais preenchidas automaticamente.</span></div></div>}
    </form></section>
  </main>;
}
