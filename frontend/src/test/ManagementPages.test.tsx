...
vi.mock("../api", async () => {
  ...
  return { ...actual, api: vi.fn(async (path: string) => {
    ...
    if (path.includes("/channels")) return Array.from({ length: 32 }, (_, index) => ({ id: index + 1, device_id: 1, channel: index + 1, name: `Sensor ${index + 1}`, enabled: index < 8, sensor_type: "K", unit: "°C", correction_offset: 0, warning_limit: 70, critical_limit: 80, color: "#3667E9" }));
...
test("lista os 32 canais configuráveis com valor e qualidade atuais", async () => {
  ...
  expect(await screen.findByText("Sensor 32")).toBeInTheDocument();
  expect(screen.getByText("CH32")).toBeInTheDocument();
  expect(screen.getAllByText("Valor atual · unavailable")).toHaveLength(32);
});
...
test("apresenta eventos retornados pelos filtros", async () => {
  ...
  expect(screen.getByText("Conexão", { selector: ".badge" })).toBeInTheDocument();
});
