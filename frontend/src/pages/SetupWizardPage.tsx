...
const steps = ["Boas-vindas", "Ambiente", "Administrador", "Equipamentos", "Associação", "Confirmação"];
...
export default function SetupWizardPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="PRIMEIRO USO" title="Configuração inicial" description="Prepare o acesso do laboratório, identifique os equipamentos e valide a comunicação." />
      ...
      <Panel className="wizard-panel">
        {step === 0 && <div className="wizard-copy"><ShieldCheck /><h2>Bem-vindo ao ThermoPower Monitor</h2><p>Este assistente prepara o ambiente local, confirma o administrador e orienta a associação segura dos instrumentos do laboratório.</p><button className="button primary" onClick={() => setStep(1)}>Começar <ChevronRight /></button></div>}
        ...
        {step === 3 && <div className="wizard-copy"><Usb /><h2>Equipamentos USB / COM</h2>{!ports.length ? <Empty title="Nenhuma porta encontrada" text="Conecte os instrumentos e confirme os drivers no Windows; você poderá voltar a esta etapa depois." /> : <div className="usb-grid">{ports.map((item) => <button className={`usb-choice ${port === item.port ? "selected" : ""}`} onClick={() => setPort(item.port)} key={item.port}><strong>{item.port}</strong><span>{item.product || item.description}</span><Badge tone={item.status === "available" ? "success" : "warning"}>{item.status === "available" ? "Disponível" : "Verificar"}</Badge></button>)}</div>}<div className="wizard-actions"><button className="button secondary" disabled={busy} onClick={() => void refresh()}>Atualizar</button><button className="button primary" onClick={() => setStep(4)}>Continuar <ChevronRight /></button></div></div>}
        ...
        {step === 5 && <div className="wizard-copy"><ShieldCheck /><h2>Teste final</h2><label className="field"><span>Equipamento</span><select value={deviceId} onChange={(event) => setDeviceId(Number(event.target.value))}>{devices.map((device) => <option value={device.id} key={device.id}>{device.name}</option>)}</select></label><button className="button secondary" disabled={!deviceId || busy} onClick={() => void runTest()}>Executar teste em etapas</button>{test && <div className="test-steps">{test.stages?.map((stage: any) => <div key={stage.key}><Badge tone={stage.status === "passed" ? "success" : stage.status === "failed" ? "danger" : "neutral"}>{stage.status}</Badge><span>{stage.label}</span></div>)}</div>}<p className="hint">Confirme as etapas de conexão, configuração e leitura antes de concluir.</p><button className="button primary" onClick={finish}>Concluir configuração <Check /></button></div>}
