...
export default function DashboardPage() {
  ...
  const visibleReadings = useMemo(() => {
    const source = (visualPaused ? visualSnapshot : readings).filter((reading) =>
      [electricalDeviceId, temperatureDeviceId].includes(reading.device_id)
      && (!activeSession || reading.session_id === activeSession.id));
    if (!windowSeconds) return source;
    const cutoff = Date.now() - windowSeconds * 1000;
    return source.filter((reading) => parseUtcTimestamp(reading.received_timestamp ?? reading.timestamp) >= cutoff);
  }, [readings, visualPaused, visualSnapshot, windowSeconds, electricalDeviceId, temperatureDeviceId, activeSession]);
  ...
  const latestTemperature = combined.latestTemperature ?? statuses[temperatureDeviceId]?.latest_reading;
  const latestElectrical = combined.latestElectrical ?? statuses[electricalDeviceId]?.latest_reading;
  const powerPeak = seriesPeak(combined.chartData, ["power"]);
  const thermalPeak = seriesPeak(combined.chartData, Array.from({ length: 32 }, (_, i) => `t${i + 1}`));
  ...
  const temperatures = validTemperatures(latestTemperature);
  ...
  const visibleTemperatureChannels = [...new Set(combined.temperatureReadings.flatMap((reading) => reading.temperatures_c.flatMap((value, index) => value == null ? [] : [index + 1])))].sort((a, b) => a - b);
  ...
  const statusCard = (label: string, device: Device | undefined, reading: Reading | undefined) => {
    ...
    const sourceFailed = status?.state === "error" || Boolean(status?.last_error) || Boolean(status?.persistent_failure);
    ...
    return <div className="source-status">
      ...
      <div><span>{label}</span><strong>{device?.name ?? "Não selecionado"}</strong>
        <small>{status?.persistent_failure ? "Falha persistente - tentando reconectar automaticamente" : sourceFailed ? "Falha nesta fonte" : status?.state === "recovering" ? "Reconectando automaticamente..." : status?.state === "connecting" ? "Conectando..." : status?.connected ? "Conectado" : "Desconectado"} · Última leitura válida {elapsedLabel(reading?.timestamp ?? status?.last_message_at, now)}</small>
